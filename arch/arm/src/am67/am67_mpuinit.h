 ...
 *   P:RW   U:RW
 *
 * NOTE: This region must NOT be marked Shareable. On the Cortex-R5,
 * exclusive
 * accesses (LDREX/STREX) to a Shareable Normal region are routed to an
 *  external
 * global exclusive monitor on the interconnect. The K3 NoC path to DDR for
 *  this
 * R5F does not implement one, so such accesses take a synchronous external
 *  data
 * abort (DFSR encodes external abort). Single-core NuttX never issues LDREX
 *  on
 * DDR, so this stayed latent; PX4's C++ std::atomic (and any ldrex-based
 *  lock)
 ...
 * R5F<->A53 rpmsg coherency, which is handled by cache maintenance,
 * not the S
